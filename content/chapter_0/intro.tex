...
\section{The origin}
The project of self  camera organisation  has been initiated by a partnership between the CISIR and the previously called Le2i. The project has to aim the research for UAV work cooperation for smart surveillance and control.  
The project start with a basic idea to develop a smart system composed by camera mounted on UAVs. 
The first step, is to manage the different position of a UAVs inside the area to control. The aim here is to find the best point of view for the surveillance.
For that is important to find the relevant waypoints and create a shorter path passing by this waypoints in order to can efficiently control a given zone. Our concern here is to find the strategic positions (for the waypoints) to control the area despite the environmental constraints. The constraint can be various but as discussed in detail in the following section the shape of the area, the UAV limitation, the camera properties , are some example of constraints. 
Estimating efficiently the path plan with strategic waypoint poses is an important technical lock for create a complete and autonomous system of smart cameras mounted UAVs. 
