...
The following section is dedicated to the Coverage Path Planning (named CPP). Until now, the work presented was focus on optimizing the position and orientation of a fix number of cameras to cover a vast and complex area. 
The method introduced using GA or a more flexible (but slower) GAPSO, give an efficient result to estimate the pose of a set of fix cameras. To find a solution for the CPP problem we propose to use the algorithms developed for the cameras positioning to find the set of useful waypoints. Instead to place several cameras at each poses estimate, the poses are considered as waypoints of a complete path. 
...
\begin{itemize}
	...
	A crucial step is to estimate the number of waypoints. A wrong estimation with a too high or to low will cause au  bad area coverage or a to complex computation.
	\item Waypoints positioning : 
	The waypoints positioning optimization is the more crucial step. As already discuses numerous solution has been studied to optimize the pose of the cameras depending several constraints. Based on it and the experiment made until now, an efficient algorithms has been chosen and adapted to pose estimate each waypoint (see  Section  \ref{sec:hybridGAPSO}). 
...
The method proposed have the advantage to optimize independently all the waypoints and the path plan. This method is also not systematically based on a sweep and allows to have a shorter path with the same starting point and ending point. 
...
To do so, a two-step procedure has been implemented. Based on the pose optimization for a fixed number of waypoints introduce in the precedent Chapter \ref{chap:waypointPoseExp}. 
...
Ponnambalam et al 172* \cite{172*} propose to use a multi-objective GA to optimize the TSP. Moreover then, the GA used Ponmambalam et al 172* provide the GA set-up (the mutation rate, population size,...). DAVIES et al \cite{56*davies2006} propose also to use the GA to solve the TSP applied on robotic with obstacles constraint. \\
...
Once the waypoints position estimated and the shorter path passing by all the waypoints computed it is interesting to can evaluate the trajectory complexity. The trajectory complexity can be a good clue of the path reliability.  It also be useful to compare our solution to the more classical sweep trajectory.
...
The method proposed to optimize the CPP problem is tested during different experimentation. The differents  experimentation bring out the advantage and the limit of the developed method. The experiment are structured in sub-section with show the method and algorithms in more and more complex experimentations.
...
Thanks to this first experiment the path planning proposed appear much more appropriate. The path planning obtained by our method proposed a shorter path, 881.6 pixel length when  the path by sweep is  at 1137 pixel length. The path proposed by the sweep is longer notably due to the return to the starting point, but not only. The junction between the start and finish of the swept path is 245 pixel long, with mean the path useful to fully cover the area is around 892 pixel long. Moreover the proposed path planning in addition then the shorter path propose also a better trajectory complexity  with 65.22$^\circ$ instead to 97.81$^\circ$ for the path with sweep.
Finally the proposed solution allows a better path planning thanks to this optimized waypoints. Despite a better path plan efficiency few points of the area are not covered in contrary than the traditional swept method.
...
To focus on a bigger area the experiment for outdoor waypoints positioning for vast and complex area using  a mask  is reused as in Section \ref{sec:fey_map}. This experiment has the advantage to be vast and complex enough for the comparison between the sweeping method and the proposed method.\\
...
The path plan using a sweeping method is visible in the Figure \figref{subfig:FleytfullRoomPath}. In order to cover completely the area several overlap and un-interesting region (the black region in Figure \figref{subfig:FleytfullRoomPath}) has to be covered. 

The solution proposed by optimizing in a first time the waypoints position and in second time the path passing by the waypoints is applied. The optimization is made for a set of 80 waypoints which must coverage over the 98\% of the area. This threshold is reached after a 3101 generations (see in \figref{subfig:FleyPathPlanning}). Once the waypoints optimized (in $x;y;z$ and roll) the path plan is computed using the method based on the TSP  (presented in the Section \ref{sec:TSP2}). The final CPP is visible in the Figure \figref{subfig:FleyPathPlan} with the different altitude represented in color.

One more time and despite the increased size and complexity of the map (with increase greatly the complexity and the number of the waypoints optimization) the proposed solution give a much better result.  The path plan is shorter 4362px for the pattern method versus 4015px for our method and  the path is also  easier to compute  in term of trajectory 82.14$^\circ$ versus 65.78$^\circ$.
...
The simple sweep path  is computed based on the camera projection at the highest altitude (see in Figure \figref{subfig:CalvissonfullRoomPath}). Our method is computed  with 110 waypoints to pose estimate. After  3634 generation for a coverage area at almost 95\% (see Figure \figref{subfig:CalvissonPathPlanning}) the path  can be computed (as explained in the section \ref{sec:sorted}).  The final CPP is shown in the Figure \figref{subfig:CalvissonPathPlan}. 

One more time the solution proposed give a shorter path compare then the swept method (8173px versus 8582px) for an high level of waypoints coverage. But the difference between the 2 path are not so important  %even more considering the  return to the starting point to the swept method (around 770px)
in term of distance but the trajectory complexity give an important advantage to the method proposed.\\
Thanks to this experiment the limit of our method begin to appears. The principal limitation is due to the difficulty to optimize numerous waypoints (as that was discussed in the Section \ref{sec:biggestMapcalWaypoint}). The increased number of waypoints associate to a high level of coverage rate required, push the GAPSO optimization to the limit . Numerous generation are necessary before to converge. On the other hand, the path plan computation appear efficient despite the increased number of waypoints and globally the solution proposed give an better CPP in term of distance and trajectory complexity for a high coverage rate.
...
%blabla figure  fort taux de couverage path plus cour et surtout trajectoire plus complexité meilleieur 
 ...
 \subsubsection{Non holonomic robot.} \label{holonomie path}

The previous indoor or outdoor simulations, assumed to have a holonomic UAV or an UAV with a camera stabilization for the roll, in order to stabilise the roll (portrait or landscape) independently then the UAV trajectory.
During this simulation we assume the UAV are not able to control the roll independently of the direction and the UAV are sufficiently agile to follow the path.
The solution proposed in this paper is adapted to the non holonomic constraints. 
In the following experimentation the roll and the altitude are fixed for the waypoint positioning, the camera size are reduced as a scare shape projection onto the ground 40x40px, corresponding to the smaller side of the camera projection. The reduction of the camera estimation, the fixed roll and also a fix altitude premiss us to make an coverage at minima of the area.
The optimisation of the waypoints are computed (as in Chapter \ref{chap:waypointPoseExp} ) with this new constraint and 
...
This experiment demonstrates by one example the ability of our proposition to do coverage path planning in a large area with a non convex shape with different constraints. The solution proposed is adaptable and flexible to the different condition. This experiment also show the non negligible gain of coverage between the waypoints positioning optimization and the final CPP with the stream coverage of the area.
...
The experiment made previously was successful despite the different optimization steps. In fact to can compute an efficient path plan several optimization (see Section \ref{sec:camerasPositioning}) has been used with an increasing number of waypoints until reaching an acceptable threshold of coverage rate (see Section \ref{sec:NonEAmethod}). When the coverage rate is reached the optimized waypoints can be refined with a PSO to have an  appropriate coverage. The final optimization is to use the TSP paradigm to compute a path passing by all the waypoints. The estimation of the number of waypoints appear as the most consumer of computation but the case shown during the experimentation the number of waypoints was fixed manual.% (due to the knowledge about each map). 
...
Also in the experiment made for non holonomy UAV the stream coverage area increase significantly compared to the waypoint coverage. This increased area covered is not taking in account until now. The idea is to try to limit the number of waypoints by using a global method to optimize the CPP problems.
...
In order to find a solution for the problem of CPP by one optimization for the waypoints positioning and the path computing the problem has to be re-adapted.
Until now, the waypoints positioning and the path planning computation were made by using 2 different GA. The idea here is to combine the cost function from the waypoints positioning and the path planning computation to have an appropriate and global cost function.
...
\begin{itemize}
	...
	\item Second is the path distance. The path distance was already used to estimate the path passing  by all the given waypoints during the path computation (as in Section \ref{sec:TSP2}). 
\end{itemize}
...
To formulate properly the CPP problem is essential to can estimate the area covered by the UAV during path passing by all the optimized waypoints. For that the waypoints and the path must be knew. 
...
\end{mfigures}   
 ...
 Finally the new cost function will integrate the covered area by the path passing  by all the waypoints and the distance of this path. The goal is to have an cost function appropriate to maximize the area covered with the shorter path at same time.
...
One more time the GA is used associate to the PSO. If the EA (especially the GA) are used here is for few main reasons. The GA was the best algorithms to solve the TSP problem and also well adapted to the waypoints positioning (even more with an GAPSO). The last reason is the knowledge grab about the GA PSO until know make the implementation fast and easy.
 The solution adopted is to use the GAPSO as introduced in \ref{sec:hybridGAPSO} associate to the newly adapted cost function (see Paragraph \ref{par:CPPestimation}).
		...
		\subsection{Result  and consequancis }
...
\end{mfigures}
	...
	Based on the new problems formulation and the use of an appropriate GAPSO directly inherited then the waypoints positioning (see Chapter \ref{chap:waypointPoseExp})few experiments has been made. The experiment proposed here is based on the map presented in the Section \ref{sec:maskGAPSO}.
% The experiment flow: 
...
The main advantage of this formulation is the beside to optimize the CPP problem in one unique optimisation is the reducing amount of waypoint compared to the waypoint positioning optimization. In fact for the map proposed only  5 waypoints are required to cover  the area (see Figure \figref{subfig:CPPTorcymapGAPSO}) compare then the waypoints positioning optimisation require 25waypoints (see Figure \figref{subfig:coverageGAPSOTorcy}). Reducing the number of waypoints needed to cover an area is essential to have an better and faster optimization process. The reducing amount of the number of waypoints is even more important when the problem is complicated by the position order of the waypoints in a solution. 
	...
	The fast studies of these experiments made, show some limit of using the unique optimization for the problem of CPP. In the other hand, the formulation despite the inconclusive, but promising result appear interesting and  the cause of the inconclusive result are explored.
	...
	\paragraph*{Why inconclusive result?}
		...
		The inconclusive result of the experiment can be from a the algorithms used, the cost function or the increased complexity and search space.
		\begin{itemize}
		\item The algorithms: As that was introduced the GAPSO  with a similar set-up then the previous experiment for the waypoints positioning (see Chapitre \ref{chap:waypointPoseExp}) are used. Few inconclusive tests has been done to slightly modify the set-up of the GAPSO by adding more mutation for example. The inconclusive preliminary result push to consider the GAPSO proposed previously  good enough and the other reason has to be explored.
		\item The cost function: The cost function presented in the Equation \ref{eq:CostF2} is the cost function the more appropriate due to our experiment. Despite that the cost function has some coefficient which they were found empirically. The coefficient could  an interesting track which must be thorough in order to reduce their uncertainty.
		\item Complexity and search space: 
		The last potential reason of the inconclusive result obtained can be the increased size of the search space and complexity. The size of the search space was discussed in the Section \ref{sec:OptimizationComplexity}. The search space estimation was made for the problem of optimizing the waypoints position and can be reused by part for the problem of CPP in unique optimization. First the Equation \ref{eq:SearchSpace} can be simplified.  
		\begin{equation} \label{eq:SearchSpaceCPP}
		 Sp=(W\times H)  
		\end{equation}
		Where $W$ and $H$ are the size as width and height of the area to cover. $Sp$  is simplified due to the reduced number of dimension optimized. In fact the altitude is fixed and identical for all the waypoints as the pant tilt and roll depend then the path.
		Despite the important decreasing of a search space for a waypoint, the global search space for the CPP increase dramatically  due to the number of waypoint and  the ordered constraint.
...
The conclusion is despite the low quality of the answer obtained with this methodologies the interesting track and possible gain must push the research to continue to explore the GAPSO optimization to solve the problem of coverage path planning problem as a one optimization problem. But caution the increased search space can became break of the optimization despite the reducing of number of dimension  to optimize. In fact the number of dimension is reduced by fixing the altitude and the the camera orientation and moreover with this  unique optimization  the number of required waypoints are greatly reduced. 
The main risk of using  an unique optimization for the problems of CPP is the complexity and the associate search space. In fact among the reason to explain the for the inconclusive result the complexity due to the fusion of 2 NP-hard and NP-complete problem could be the cause. \\
