...
The constraint (or secondary objectives) are numerous and varied, where just few of them has been introduced (the more common and interesting). Among it, some of them are closely related and can be interconnected. The secondary constrains can be associate as in \cite{33*reddy2012} for example where the targets coverage, luminosity, resolution, are associate to find the best cameras position with maximize the coverage of the area and the  tracking target with good visibility condition. \\
...
The constraints can be considered as a constraint with can restrict the main objective which is the coverage of an area.
