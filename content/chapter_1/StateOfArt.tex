...
\begin{itemize}
	...
	\item Semi approximate: 
	 The semi approximate decomposition is by part based on the discretization of the space. The idea is to create a set of large cells. The width of the cells is fix and the height is relative then the area boundary (see [241*]). 
	...
	The exact cellular decomposition has continue to be studied since the survey of  Carreras and Galceran  \citep{66*galceran2013} and upgraded, as in 144*  \cite{} for propose a decomposition for concave or multiple polygons.
...
\end{itemize}	
	 The sweep can be also alternatively switch for a spiral as proposed in Jimenez et al [217*]. The proposed sweep and spiral in [217*] are adaptable depending then the context. The sweep can have two direction and for the spiral two turn side (clockwise or counter-clockwise).  The different sweep and spiral are illustrated in the Figure \figref{fig:sweepSpiral}. 
...
To optimize the TSP numerous algorithms was tested and some of them has been specially applied to schedule the cells to have the shorter path. In  An et al [60*] two algorithms was tested before to develop a third. The algorithms used are based on branch and bound. The algorithms developed in [60*] is called "Novel Previous-Next Waypoints Coverage Constraint" (PNWCC). The algorithms presented in [60*] propose at same time a schedule of each cells with also a smooth trajectory without sharp edges usable for non holonomic driving robots. 

	In the survey of Carreras et Galceran \citep{66*galceran2013} among the solutions proposed to solve the TSP is to computes an exhaustive walk trough the adjacency graph. This solution is workable only for small adjacency graph. Among the other solution proposed the Genetic Algorithm. The GA is popular to optimize the TSP and is commonly use to evaluate the influence of this parameter as in \citep{68*muhlenbein1989} \citep{80*serpell2010} [139*]. About the CPP problem the GA is also used to optimize the TSP part as in Jimenez et al [217] where after an exact cellular decomposition of a complex polygon. 
	
The GA is announced well appropriate to have an optimized solution for the TSP. 
Also the in some situation a  TSP is not realistic and some external constraint can be added as the wind effect, the turbulences, or holonomy constraint as in [102*,56*] \citep{66*galceran2013}).
...
%		\paragraph{multi robots}
		...
		This solution is relatively popular for the plan the hoover robot path as in 216*  215* 196* \citep{218*meiting2007}.
		The idea is to have an algorithms which not tried to optimize globally the path planning but have an adapted local decision with maximize the global path plan. In Luo et al [216*] the goal is to have a complete coverage by visiting all the case of a grid. To visit all the case of the grid a neural-neighborhood analysis and neural dynamic programming approaches are adapted to can efficiently CPP for the robots. In Simon et Luo [196*] they propose an similar method with a neural network solution for dynamic and non planar area.
		
		The work of Lee et al [215*] propose an other solution Based on smooth spiral path. The idea is to propose to follow the boundary of the room to fully cover and use a smoothed spiral to cover the area. The solution proposed is compared to other method. The main advantage of these method is the on-line ability and the smoothed trajectory(see also [195*]).
