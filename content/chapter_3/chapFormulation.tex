...
In order to represent correctly the set of camera with the useful parameter to compute the camera projection , the precedent notation can be adapted to have a set of $V$ composed by $N$ cameras defined by their parameters $v$:
	...
	\begin{equation}\label{eq:V}
		...
		\end{split}
	\end{equation}
\noindent Where $N$ is the given number of cameras in the network. Which is also the number of cameras in the set $V$. \\
Therefore, $V$ represent a solution. $V$ contain all the position and orientation of the set of cameras for a predefined focal length, sensor size and map.\\% wish also include the potential constraint as  some restriction on the map.\\
...
That mean among all the possible combination of parameter $V$ only the one respect the constraint of the set $E$ in order to have an possible solution. where $Vs=V\subset E$.\\ !!!! to continue !!!!
