...
This chapter present first, a formal definition  of the problem based on the literature and in the second time, our formulation is proposed. The formulation proposed is adapted to optimize the problems efficient cameras coverage using camera mounted on UAV.
...
\item Third part is to add the constraint. 
\end{itemize} 

Thanks to the camera definition the area cover for one camera depending then this parameters can be computed.

The area is at same time the more evident element and the one with propose the most variety of design.
The area to cover have to be discretized  and this discrétization can be donne by diferent methode with a different sampling frequancy as is descripted in the folowing section. 
