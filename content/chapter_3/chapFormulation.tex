...
Based on the useful parameters from the camera $(f ,Sw\times Sh ,z)$ the computation to estimate the size of a camera projection ($Wr$,$Hr$) is :
...
One of the first constraint is the fixed number of cameras. This constraint as some others (detailed latter) are  useful to simplify and restrict the possibility of the problem. This constraint allows us to focus on the fine optimisation (as in \citep{22*zhao2008} where both are tested). The number of cameras is fixed at the beginning of the optimisation and no more camera will be added during the optimization process.  
